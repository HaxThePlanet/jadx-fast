		...
		for (ICodeLoader codeLoader : loadedInputs) {
			codeLoader.visitClasses(cls -> {
				try {
					...
					// SKIP FRAMEWORK AND GENERATED CLASSES
					...
					// Data Binding
					if (clsName.contains("/databinding/")
							|| clsName.contains("DataBinderMapperImpl")
							|| clsName.contains("DataBindingMapperImpl")
							|| clsName.contains("BindingAdapter")) {
						return;
					}

					// Dependency Injection - Dagger/Hilt
					if (clsName.endsWith("_Factory;")
							|| clsName.contains("_Factory$")
							|| clsName.contains("_HiltModules")
							|| clsName.contains("_HiltComponents")
							|| clsName.contains("Hilt_")
							|| clsName.contains("DaggerApplicationComponent")
							|| clsName.contains("_MembersInjector")
							|| clsName.contains("_ProvideFactory")
							|| clsName.contains("_Injector")
							|| clsName.contains("_Module_")) {
						return;
					}

					// Butterknife
					if (clsName.contains("_ViewBinding")
							|| clsName.contains("_ViewBinder")
							|| clsName.contains("$ViewBinder")
							|| clsName.contains("$ViewInjector")) {
						return;
					}

					// Flutter
					if (clsName.startsWith("Lio/flutter/")
							|| clsName.contains("FlutterApplication")
							|| clsName.contains("FlutterActivity")
							|| clsName.contains("FlutterFragment")
							|| clsName.contains("GeneratedPluginRegistrant")) {
						return;
					}
					...
					// Unity
					if (clsName.startsWith("Lcom/unity3d/")
							|| clsName.startsWith("Lcom/unity/")
							|| clsName.startsWith("Lbitter/jnibridge/")
							|| clsName.startsWith("Lorg/fmod/")
							|| clsName.startsWith("Lcom/google/androidgamesdk/")) {
						return;
					}

					// Cordova/PhoneGap
					if (clsName.startsWith("Lorg/apache/cordova/")
							|| clsName.startsWith("Lorg/crosswalk/engine/")
							|| clsName.startsWith("Lcom/ionicframework/")) {
						return;
					}

					// Xamarin
					if (clsName.startsWith("Lmono/")
							|| clsName.startsWith("Lxamarin/")
							|| clsName.startsWith("Landroid/runtime/")
							|| clsName.startsWith("Lmd5")
							|| clsName.startsWith("Lmd51")) {
						return;
					}

					// Cocos2D
					if (clsName.startsWith("Lorg/cocos2dx/")
							|| clsName.startsWith("Lcom/chukong/")) {
						return;
					}

					// Database/ORM - Room
					if (clsName.endsWith("_Impl;")
							|| clsName.contains("_Impl$")
							|| clsName.contains("RoomDatabase_Impl")
							|| clsName.contains("Dao_Impl")) {
						return;
					}

					// GreenDAO
					if (clsName.contains("DaoMaster")
							|| clsName.contains("DaoSession")) {
						return;
					}

					// Realm
					if (clsName.startsWith("Lio/realm/")
							|| clsName.contains("RealmProxy")
							|| clsName.contains("RealmModule")) {
						return;
					}

					// Protocol Buffers
					if (clsName.endsWith("Proto;")
							|| clsName.contains("ProtoOrBuilder")
							|| clsName.contains("OuterClass$")) {
						return;
					}

					// Gson
					if (clsName.contains("TypeAdapter$")
							|| clsName.contains("TypeAdapterFactory")) {
						return;
					}

					// Moshi
					if (clsName.contains("JsonAdapter")
							|| clsName.contains("MoshiJsonAdapter")) {
						return;
					}

					// Testing frameworks
					if (clsName.startsWith("Lorg/junit/")
							|| clsName.startsWith("Lorg/mockito/")
							|| clsName.startsWith("Lorg/hamcrest/")
