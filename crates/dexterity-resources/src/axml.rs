...
use crate::constants::*;
use crate::android_res_map::ANDROID_RES_MAP;
...
impl AxmlParser {
    ...
    fn get_attribute_name(&mut self, idx: u32) -> String {
        ...
        if (idx as usize) < self.resource_ids.len() {
            ...
            if let Some(name) = ANDROID_RES_MAP.get(&res_id).map(|s| s.as_str()) {
    ...
    fn decode_reference(&self, data: u32) -> String {
        ...
        if let Some(name) = ANDROID_RES_MAP.get(&data).map(|s| s.as_str()) {
