...
use dexterity_dex::sections::{
    ...
    MethodHandleType, VISIBILITY_BUILD, VISIBILITY_RUNTIME, VISIBILITY_SYSTEM,
};
...
use dexterity_ir::{
    ...
    ArgType, ClassData, FieldData, FieldValue, InsnType, LambdaHandleType, LambdaInfo,
    MethodData, TypeParameter,
};
...
/// Convert a DEX MethodHandleType to IR LambdaHandleType
fn convert_handle_type(handle_type: MethodHandleType) -> LambdaHandleType {
    match handle_type {
        MethodHandleType::InvokeStatic => LambdaHandleType::InvokeStatic,
        MethodHandleType::InvokeInstance => LambdaHandleType::InvokeInstance,
        MethodHandleType::InvokeConstructor => LambdaHandleType::InvokeConstructor,
        MethodHandleType::InvokeDirect => LambdaHandleType::InvokeDirect,
        MethodHandleType::InvokeInterface => LambdaHandleType::InvokeInterface,
        // Field handles - shouldn't occur in lambdas, but map to InvokeStatic as fallback
        MethodHandleType::StaticPut
        | MethodHandleType::StaticGet
        | MethodHandleType::InstancePut
        | MethodHandleType::InstanceGet => LambdaHandleType::InvokeStatic,
    }
}

/// Resolve InvokeCustom call site to populate LambdaInfo
///
/// This parses the CallSite data from the DEX file to extract lambda/method reference
/// information. CallSites for lambdas typically contain:
/// - [0] MethodHandle for bootstrap method (LambdaMetafactory.metafactory)
/// - [1] String - method name in functional interface (e.g., "apply", "run")
/// - [3] MethodType - SAM method type (lambda parameter types and return type)
/// - [4] MethodHandle - implementation method
///
/// Returns None if the call site cannot be resolved or is not a lambda invocation.
fn resolve_lambda_info(dex: &DexReader, call_site_idx: u32) -> Option<LambdaInfo> {
    // Get the call site data
    let call_site = dex.get_call_site(call_site_idx).ok()?;

    // Check if this looks like a lambda invocation (has enough values)
    if !call_site.is_lambda_invoke() {
        return None;
    }

    // Get implementation method handle (at index 4)
    let impl_handle_idx = call_site.get_impl_method_handle()?;
    let impl_handle = dex.get_method_handle(impl_handle_idx).ok()?;

    // Get interface method name (at index 1, e.g., "apply", "run", "accept")
    let method_name_idx = call_site.get_method_name()?;
    let interface_method_name = dex.get_string(method_name_idx).ok()?;

    // Get implementation method details
    let impl_method = dex.get_method(impl_handle.field_or_method_idx as u32).ok()?;
    let impl_class = impl_method.class_type().ok()?;
    let impl_method_name = impl_method.name().ok()?;

    // Get lambda parameter types and return type from SAM method type (at index 3)
    let (lambda_param_types, lambda_return_type) = if let Some(sam_proto_idx) = call_site.get_sam_method_type() {
        if let Ok(proto) = dex.get_proto(sam_proto_idx) {
            let param_types: Vec<ArgType> = proto.parameters().ok()
                .map(|params| params.iter().map(|p| parse_type_descriptor(p)).collect())
                .unwrap_or_default();
            let return_type = proto.return_type().ok()
                .map(|r| parse_type_descriptor(&r))
                .unwrap_or(ArgType::Void);
            (param_types, return_type)
        } else {
            (Vec::new(), ArgType::Void)
        }
    } else {
        (Vec::new(), ArgType::Void)
    };

    // Get implementation method proto to calculate captured arg count
    let impl_proto = impl_method.proto().ok();
    let impl_param_count = impl_proto.and_then(|p| p.parameters().ok())
        .map(|params| params.len())
        .unwrap_or(0);

    // Captured args = impl method params - lambda params
    // (lambda params come from the SAM interface, captured vars are extra)
    let captured_arg_count = impl_param_count.saturating_sub(lambda_param_types.len());

    // Determine if this should use method reference syntax (::) vs lambda syntax (->)
    // Method references are used when the implementation is NOT a synthetic lambda$ method
    // Synthetic lambda methods are generated by the compiler for actual lambda expressions
    let is_synthetic_lambda = impl_method_name.starts_with("lambda$");
    let use_method_ref = !is_synthetic_lambda;

    // For constructor references (Class::new), check if it's a constructor
    let is_constructor = impl_method_name == "<init>";

    Some(LambdaInfo {
        handle_type: convert_handle_type(impl_handle.handle_type),
        impl_method_idx: impl_handle.field_or_method_idx as u32,
        impl_class: strip_descriptor(&impl_class).to_string(),
        impl_method_name: if is_constructor {
            "new".to_string() // Use "new" for constructor references
        } else {
            impl_method_name.to_string()
        },
        interface_method_name: interface_method_name.to_string(),
        use_method_ref,
        inline_body: is_synthetic_lambda,
        captured_arg_count,
        lambda_param_types,
        lambda_return_type,
    })
...
pub fn load_method_instructions(method: &mut MethodData, dex: &DexReader) -> Result<()> {
    ...
    // Resolve lambda/method reference information for InvokeCustom instructions
    // This populates LambdaInfo from CallSite data in the DEX file
    for insn in &mut instructions {
        if let InsnType::InvokeCustom { call_site_idx, lambda_info, .. } = &mut insn.insn_type {
            if lambda_info.is_none() {
                *lambda_info = resolve_lambda_info(dex, *call_site_idx);
            }
        }
    }
