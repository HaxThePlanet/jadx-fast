...
/// Error type for region building operations (like JADX's JadxOverflowException)
#[derive(Debug, Clone)]
pub enum RegionError {
    /// Region stack size limit exceeded
    StackOverflow { limit: usize },
    /// Too many regions created (likely infinite loop)
    RegionsLimitReached { limit: usize },
    /// Region contents limit exceeded
    ContentsLimitReached { block_id: u32, limit: usize },
    /// Iteration budget exhausted
    IterationLimitReached { used: usize, budget: usize, blocks: usize },
}

impl std::fmt::Display for RegionError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RegionError::StackOverflow { limit } => {
                write!(f, "Region stack size limit reached ({})", limit)
            }
            RegionError::RegionsLimitReached { limit } => {
                write!(f, "Regions count limit reached ({})", limit)
            }
            RegionError::ContentsLimitReached { block_id, limit } => {
                write!(f, "Region contents limit reached ({}) at block {}", limit, block_id)
            }
            RegionError::IterationLimitReached { used, budget, blocks } => {
                write!(
                    f,
                    "Iteration limit reached: {} iterations (budget: {} = 20 × {} blocks)",
                    used, budget, blocks
                )
            }
        }
...
/// Exception type for a catch handler
#[derive(Debug, Clone, PartialEq, Eq)]
pub enum ExceptionType {
    /// Catches a specific exception type (e.g., "Ljava/lang/Exception;")
    Specific(String),
    /// Catches all exceptions (catch-all / finally)
    CatchAll,
}

impl ExceptionType {
    /// Returns true if this is a catch-all handler
    pub fn is_catch_all(&self) -> bool {
        matches!(self, ExceptionType::CatchAll)
    }

    /// Returns the exception type string if specific, None if catch-all
    pub fn as_specific(&self) -> Option<&str> {
        match self {
            ExceptionType::Specific(s) => Some(s),
...
pub struct HandlerInfo {
    /// Exception type for this handler
...
pub fn build_regions(cfg: &CFG) -> Result<Region, RegionError> {
    build_regions_with_try_catch(cfg, &[])
}

/// Build regions with try-catch block information from IR
pub fn build_regions_with_try_catch(cfg: &CFG, try_blocks: &[dexterity_ir::TryBlock]) -> Result<Region, RegionError> {
...
///
/// Returns `Err(RegionError)` if limits are exceeded (like JADX's JadxOverflowException).
pub fn build_regions_with_method_flags(cfg: &CFG, try_blocks: &[dexterity_ir::TryBlock], method_flags: u32) -> Result<Region, RegionError> {
...
pub fn mark_duplicated_finally(cfg: &mut CFG, try_blocks: &[dexterity_ir::TryBlock]) {
    ...
    for try_info in &tries {
        ...
        let Some(catch_all_idx) = try_info.handlers.iter().position(|h| h.exception_type.is_catch_all()) else {
            continue;
        };
...
fn merge_multi_catch_handlers(handlers: Vec<HandlerInfo>) -> Vec<HandlerInfo> {
    ...
    for (_block, mut group) in by_block {
        if group.len() == 1 {
            ...
            if let ExceptionType::Specific(t) = std::mem::replace(&mut base.exception_type, ExceptionType::CatchAll) {
                all_types.push(t);
            }
            for other in group {
                if let ExceptionType::Specific(t) = other.exception_type {
            ...
            if all_types.len() > 1 {
                ...
                base.exception_type = ExceptionType::Specific(all_types.join("|"));
            } else if !all_types.is_empty() {
                base.exception_type = ExceptionType::Specific(all_types.remove(0));
            }
...
impl<'a> RegionBuilder<'a> {
    ...
    fn process_try_catch(&mut self, try_info: &TryInfo) -> (Region, Option<u32>) {
        ...
        let catch_handlers: Vec<CatchHandler> = try_info
            ...
            .map(|(_, handler)| {
                ...
                CatchHandler {
                    exception_types,
                    catch_all: handler.exception_type.is_catch_all(),
    ...
    fn detect_finally_block(&mut self, try_info: &TryInfo) -> (Option<Box<Region>>, Option<usize>) {
        ...
        let catch_all_idx = try_info.handlers.iter().position(|h| h.exception_type.is_catch_all());
