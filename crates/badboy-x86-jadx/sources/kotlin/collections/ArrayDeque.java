...
public final class ArrayDeque<E>  extends kotlin.collections.AbstractMutableList<E> {
    ...
    private final void removeRangeShiftPreceding(int fromIndex, int toIndex) {
        ...
        copyCount = fromIndex;
        while (copyCount > 0) {
            i3 = Math.min(copyCount, Math.min(copyFromIndex + 1, copyToIndex + 1));
            ArraysKt.copyInto(this.elementData, this.elementData, i14++, i15++, copyFromIndex + 1);
            copyFromIndex = negativeMod(copyFromIndex - i3);
            copyToIndex = negativeMod(copyToIndex - i3);
            copyCount -= i3;
        }
    }
    ...
    public boolean removeAll(Collection<? extends Object> elements) {
        ...
        final int i3 = 0;
        if (!obj.isEmpty()) {
            positiveMod = elementData3.length == 0 ? 1 : newTail$iv;
            if (positiveMod != 0) {
            } else {
    ...
    public E removeAt(int index) {
        ...
        if (index < size2 >>= i6) {
            if (positiveMod2 >= this.head) {
                ...
                ArraysKt.copyInto(this.elementData, this.elementData, i6, elementData3, positiveMod2);
                this.elementData[elementData3] = this.elementData[length3 -= i6];
            ...
            if (positiveMod2 <= positiveMod) {
                ...
                this.elementData[length2 -= i6] = this.elementData[elementData3];
                ArraysKt.copyInto(this.elementData, this.elementData, elementData3, i6, positiveMod + 1);
            }
