...
public final class ConstrainedOnceSequence<T>  implements kotlin.sequences.Sequence<T> {
    ...
    private final AtomicReference<kotlin.sequences.Sequence<T>> sequenceRef;
    public ConstrainedOnceSequence(kotlin.sequences.Sequence<? extends T> sequence) {
        Intrinsics.checkNotNullParameter(sequence, "sequence");
        super();
        AtomicReference atomicReference = new AtomicReference(sequence);
        this.sequenceRef = atomicReference;
    }

    public Iterator<T> iterator() {
        Object andSet = this.sequenceRef.getAndSet(0);
        if ((Sequence)andSet == null) {
        } else {
            return (Sequence)andSet.iterator();
        }
        IllegalStateException sequence = new IllegalStateException("This sequence can be consumed only once.");
        throw sequence;
    }
}
