...
public final class DurationJvmKt {
    ...
    public static final String formatToExactDecimals(double value, int decimals) {
        int length;
        Object formatForDecimals;
        if (obj5 < precisionFormats.length) {
            length = DurationJvmKt.precisionFormats[obj5];
            formatForDecimals = length.get();
            if (formatForDecimals == null) {
                int i = 0;
                length.set(DurationJvmKt.createFormatForDecimals(obj5));
            } else {
                Intrinsics.checkNotNull(formatForDecimals);
            }
        } else {
            formatForDecimals = DurationJvmKt.createFormatForDecimals(obj5);
        }
        String format = formatForDecimals.format(value);
