package kotlinx.coroutines.channels;

import java.util.concurrent.atomic.AtomicReferenceArray;
import kotlin.Metadata;
import kotlin.coroutines.CoroutineContext;
import kotlin.jvm.internal.Intrinsics;
import kotlinx.coroutines.internal.OnUndeliveredElementKt;
import kotlinx.coroutines.internal.Segment;

/* loaded from: classes5.dex */
@Metadata(d1 = "\u0000L\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\t\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0008\n\u0002\u0008\u0005\n\u0002\u0018\u0002\n\u0002\u0010\u0000\n\u0002\u0008\u0004\n\u0002\u0010\u000b\n\u0002\u0008\u0005\n\u0002\u0010\u0002\n\u0002\u0008\u000b\n\u0002\u0010\u0003\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\r\u0008\u0000\u0018\u0000*\u0004\u0008\u0000\u0010\u00012\u000e\u0012\n\u0012\u0008\u0012\u0004\u0012\u0002H\u00010\u00000\u0002B5\u0012\u0006\u0010\u0003\u001a\u00020\u0004\u0012\u000e\u0010\u0005\u001a\n\u0012\u0004\u0012\u00028\u0000\u0018\u00010\u0000\u0012\u000e\u0010\u0006\u001a\n\u0012\u0004\u0012\u00028\u0000\u0018\u00010\u0007\u0012\u0006\u0010\u0008\u001a\u00020\t¢\u0006\u0002\u0010\nJ)\u0010\u0014\u001a\u00020\u00152\u0006\u0010\u0016\u001a\u00020\t2\u0008\u0010\u0017\u001a\u0004\u0018\u00010\u00102\u0008\u0010\u0018\u001a\u0004\u0018\u00010\u0010H\u0000¢\u0006\u0002\u0008\u0019J\u0015\u0010\u001a\u001a\u00020\u001b2\u0006\u0010\u0016\u001a\u00020\tH\u0000¢\u0006\u0002\u0008\u001cJ!\u0010\u001d\u001a\u0004\u0018\u00010\u00102\u0006\u0010\u0016\u001a\u00020\t2\u0008\u0010\u001e\u001a\u0004\u0018\u00010\u0010H\u0000¢\u0006\u0002\u0008\u001fJ\u0017\u0010 \u001a\u00028\u00002\u0006\u0010\u0016\u001a\u00020\tH\u0000¢\u0006\u0004\u0008!\u0010\"J\u0017\u0010#\u001a\u0004\u0018\u00010\u00102\u0006\u0010\u0016\u001a\u00020\tH\u0000¢\u0006\u0002\u0008$J\"\u0010%\u001a\u00020\u001b2\u0006\u0010\u0016\u001a\u00020\t2\u0008\u0010&\u001a\u0004\u0018\u00010'2\u0006\u0010(\u001a\u00020)H\u0016J\u0016\u0010*\u001a\u00020\u001b2\u0006\u0010\u0016\u001a\u00020\t2\u0006\u0010+\u001a\u00020\u0015J\u0017\u0010,\u001a\u00028\u00002\u0006\u0010\u0016\u001a\u00020\tH\u0000¢\u0006\u0004\u0008-\u0010\"J\u001a\u0010.\u001a\u00020\u001b2\u0006\u0010\u0016\u001a\u00020\t2\u0008\u0010/\u001a\u0004\u0018\u00010\u0010H\u0002J\u001f\u00100\u001a\u00020\u001b2\u0006\u0010\u0016\u001a\u00020\t2\u0008\u0010/\u001a\u0004\u0018\u00010\u0010H\u0000¢\u0006\u0002\u00081J\u001f\u00102\u001a\u00020\u001b2\u0006\u0010\u0016\u001a\u00020\t2\u0006\u00103\u001a\u00028\u0000H\u0000¢\u0006\u0004\u00084\u00105R\u0016\u0010\u000b\u001a\n\u0012\u0004\u0012\u00028\u0000\u0018\u00010\u0007X\u0082\u0004¢\u0006\u0002\n\u0000R\u0017\u0010\u0006\u001a\u0008\u0012\u0004\u0012\u00028\u00000\u00078F¢\u0006\u0006\u001a\u0004\u0008\u000c\u0010\rR\u0011\u0010\u000e\u001a\n\u0012\u0006\u0012\u0004\u0018\u00010\u00100\u000fX\u0082\u0004R\u0014\u0010\u0011\u001a\u00020\t8VX\u0096\u0004¢\u0006\u0006\u001a\u0004\u0008\u0012\u0010\u0013¨\u00066", d2 = {"Lkotlinx/coroutines/channels/ChannelSegment;", "E", "Lkotlinx/coroutines/internal/Segment;", "id", "", "prev", "channel", "Lkotlinx/coroutines/channels/BufferedChannel;", "pointers", "", "(JLkotlinx/coroutines/channels/ChannelSegment;Lkotlinx/coroutines/channels/BufferedChannel;I)V", "_channel", "getChannel", "()Lkotlinx/coroutines/channels/BufferedChannel;", "data", "Lkotlinx/atomicfu/AtomicArray;", "", "numberOfSlots", "getNumberOfSlots", "()I", "casState", "", "index", "from", "to", "casState$kotlinx_coroutines_core", "cleanElement", "", "cleanElement$kotlinx_coroutines_core", "getAndSetState", "update", "getAndSetState$kotlinx_coroutines_core", "getElement", "getElement$kotlinx_coroutines_core", "(I)Ljava/lang/Object;", "getState", "getState$kotlinx_coroutines_core", "onCancellation", "cause", "", "context", "Lkotlin/coroutines/CoroutineContext;", "onCancelledRequest", "receiver", "retrieveElement", "retrieveElement$kotlinx_coroutines_core", "setElementLazy", "value", "setState", "setState$kotlinx_coroutines_core", "storeElement", "element", "storeElement$kotlinx_coroutines_core", "(ILjava/lang/Object;)V", "kotlinx-coroutines-core"}, k = 1, mv = {1, 8, 0}, xi = 48)
public final class ChannelSegment<E>  extends Segment<kotlinx.coroutines.channels.ChannelSegment<E>> {

    private final kotlinx.coroutines.channels.BufferedChannel<E> _channel;
    private final AtomicReferenceArray data;
    public ChannelSegment(long id, kotlinx.coroutines.channels.ChannelSegment<E> prev, kotlinx.coroutines.channels.BufferedChannel<E> channel, int pointers) {
        super(id, prev, (Segment)channel, obj7);
        this._channel = pointers;
        AtomicReferenceArray atomicReferenceArray = new AtomicReferenceArray(sEGMENT_SIZE *= 2);
        this.data = atomicReferenceArray;
    }

    private final void setElementLazy(int index, Object value) {
        this.data.lazySet(index * 2, value);
    }

    @Override // kotlinx.coroutines.internal.Segment
    public final boolean casState$kotlinx_coroutines_core(int index, Object from, Object to) {
        return ChannelSegment$$ExternalSyntheticBackportWithForwarding0.m(this.data, i++, from, to);
    }

    @Override // kotlinx.coroutines.internal.Segment
    public final void cleanElement$kotlinx_coroutines_core(int index) {
        setElementLazy(index, 0);
    }

    @Override // kotlinx.coroutines.internal.Segment
    public final Object getAndSetState$kotlinx_coroutines_core(int index, Object update) {
        return this.data.getAndSet(i++, update);
    }

    public final kotlinx.coroutines.channels.BufferedChannel<E> getChannel() {
        final kotlinx.coroutines.channels.BufferedChannel _channel = this._channel;
        Intrinsics.checkNotNull(_channel);
        return _channel;
    }

    public final E getElement$kotlinx_coroutines_core(int index) {
        return this.data.get(index * 2);
    }

    @Override // kotlinx.coroutines.internal.Segment
    public int getNumberOfSlots() {
        return BufferedChannelKt.SEGMENT_SIZE;
    }

    @Override // kotlinx.coroutines.internal.Segment
    public final Object getState$kotlinx_coroutines_core(int index) {
        return this.data.get(i++);
    }

    @Override // kotlinx.coroutines.internal.Segment
    public void onCancellation(int index, Throwable cause, CoroutineContext context) {
        int i;
        int onUndeliveredElement;
        kotlinx.coroutines.internal.Symbol bUFFERED;
        int i2;
        Object state$kotlinx_coroutines_core;
        kotlinx.coroutines.internal.Symbol symbol2;
        boolean z;
        kotlinx.coroutines.internal.Symbol symbol;
        boolean casState$kotlinx_coroutines_core;
        int i3 = 0;
        i = index >= BufferedChannelKt.SEGMENT_SIZE ? onUndeliveredElement : i3;
        i2 = i != 0 ? index - sEGMENT_SIZE2 : index;
        final Object element$kotlinx_coroutines_core = getElement$kotlinx_coroutines_core(i2);
        state$kotlinx_coroutines_core = getState$kotlinx_coroutines_core(i2);
        while (!state$kotlinx_coroutines_core instanceof Waiter) {
            state$kotlinx_coroutines_core = getState$kotlinx_coroutines_core(i2);
            if (i != 0) {
            } else {
            }
            symbol2 = BufferedChannelKt.access$getINTERRUPTED_RCV$p();
            symbol2 = BufferedChannelKt.access$getINTERRUPTED_SEND$p();
        }
        cleanElement$kotlinx_coroutines_core(i2);
        onUndeliveredElement = channel2.onUndeliveredElement;
        if (i != 0 && onUndeliveredElement != null) {
            onUndeliveredElement = channel2.onUndeliveredElement;
            if (onUndeliveredElement != null) {
                OnUndeliveredElementKt.callUndeliveredElement(onUndeliveredElement, element$kotlinx_coroutines_core, context);
            }
        }
    }

    @Override // kotlinx.coroutines.internal.Segment
    public final void onCancelledRequest(int index, boolean receiver) {
        kotlinx.coroutines.channels.BufferedChannel channel;
        int i;
        long l;
        if (receiver) {
            getChannel().waitExpandBufferCompletion$kotlinx_coroutines_core(i2 += l);
        }
        onSlotCleaned();
    }

    public final E retrieveElement$kotlinx_coroutines_core(int index) {
        final Object element$kotlinx_coroutines_core = getElement$kotlinx_coroutines_core(index);
        final Object obj = element$kotlinx_coroutines_core;
        final int i = 0;
        cleanElement$kotlinx_coroutines_core(index);
        return element$kotlinx_coroutines_core;
    }

    @Override // kotlinx.coroutines.internal.Segment
    public final void setState$kotlinx_coroutines_core(int index, Object value) {
        this.data.set(i++, value);
    }

    public final void storeElement$kotlinx_coroutines_core(int index, E element) {
        setElementLazy(index, element);
    }
}
