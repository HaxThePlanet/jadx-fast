...
public final class TreeIterator<T>  implements Iterator<T>, KMappedMarker {
    ...
    public TreeIterator(Iterator<? extends T> rootIterator, Function1<? super T, ? extends Iterator<? extends T>> getChildIterator) {
        super();
        this.getChildIterator = getChildIterator;
    ...
    private final void prepareNextIterator(T item) {
        ...
        boolean empty;
        Iterator iterator;
        Object invoke = this.getChildIterator.invoke(item);
        if (invoke != null && (Iterator)invoke.hasNext()) {
            if (invoke.hasNext()) {
