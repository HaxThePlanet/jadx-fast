...
final class SnapshotStateKt__DerivedStateKt {
    ...
    private static final <R> R notifyObservers$SnapshotStateKt__DerivedStateKt(androidx.compose.runtime.DerivedState<?> derivedState, Function0<? extends R> block) {
        ...
        if (size > 0) {
            i$iv2 = 0;
            obj = 0;
            (DerivedStateObserver)mutableVector.getContent()[i$iv2].start(derivedState);
            while (i$iv2++ >= size) {
                obj = 0;
                (DerivedStateObserver)i$iv[i$iv2].start(derivedState);
            }
        }
        ...
        if (size2 > 0) {
            i$iv = 0;
            i2 = 0;
            (DerivedStateObserver)mutableVector2.getContent()[i$iv].done(derivedState);
            while (i$iv++ >= size2) {
                i2 = 0;
                (DerivedStateObserver)content[i$iv].done(derivedState);
            }
        }
        return block.invoke();
    }
