...
public abstract class PersistentHashMapBaseIterator<K, V, T>  implements Iterator<T>, KMappedMarker {
    ...
    private final void ensureNextEntryIsReady() {
        ...
        i2 = -1;
        while (i2 < i) {
            if (moveToNextNodeWithData(i) == i2 && this.path[i].hasNextNode()) {
            }
            if (i > 0) {
            }
            this.path[i].reset(TrieNode.Companion.getEMPTY$runtime_release().getBuffer$runtime_release(), pathLastIndex);
            i--;
            pathLastIndex = 0;
            i2 = -1;
