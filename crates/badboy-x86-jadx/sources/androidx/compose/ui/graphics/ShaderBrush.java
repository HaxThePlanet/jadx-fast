        ...
        if (this.internalShader != null) {
            if (!Size.equals-impl0(this.createdSize, obj2)) {
                if (Size.isEmpty-impl(size)) {
                    ...
                    shader = unspecified-NH-jbRc;
                    this.createdSize = size;
                }
            }
        } else {
        }
        if (!Color.equals-impl0(alpha.getColor-0d7_KjU(), shader2)) {
            alpha.setColor-8_81llA(Color.Companion.getBlack-0d7_KjU());
        }
        if (!Intrinsics.areEqual(alpha.getShader(), shader)) {
            alpha.setShader(shader);
        }
        i = Float.compare(alpha2, obj9) == 0 ? 1 : 0;
        if (i == 0) {
            alpha.setAlpha(obj9);
        }
    }
