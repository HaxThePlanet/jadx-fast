    ...
    private final void persistLayers(ScatterSet<androidx.compose.ui.graphics.layer.GraphicsLayer> layers) {
        ...
        if (requiredOsVersion != 0 && layers.isNotEmpty() && !LayerManager.isRobolectric) {
            if (layers.isNotEmpty()) {
                content = !LayerManager.isRobolectric ? postponedReleaseRequests : $this$maskEmptyOrDeleted$iv$iv$iv;
            } else {
            }
        } else {
        }
        if (content != 0) {
            ...
            $this$maskEmptyOrDeleted$iv$iv$iv2.getAndroidCanvas().setInternalCanvas(j$iv$iv.getAndroidCanvas().getInternalCanvas());
