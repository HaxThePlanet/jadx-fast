...
final class InputMethodSession {
    ...
    public final void dispose() {
        ...
        synchronized (lock) {
            i$iv = 0;
            i = 0;
            obj = (WeakReference)connections.getContent()[i$iv].get();
            do {
                i = 0;
                obj = (WeakReference)content[i$iv].get();
                if (i$iv++ < size) {
                }
                (NullableInputConnectionWrapper)(NullableInputConnectionWrapper)obj.disposeDelegate();
            } while ((NullableInputConnectionWrapper)obj != null);
