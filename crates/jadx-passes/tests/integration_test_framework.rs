...
impl IntegrationTestHelper {
    ...
    pub fn decompile_dex(&self, dex_file: &Path) -> Result<HashMap<String, String>, String> {
        ...
        let reader = DexReader::from_bytes(&dex_data)
            .map_err(|e| format!("Failed to parse DEX: {}", e))?;

        // Build IR for all classes
        let mut results = HashMap::new();

        for class_idx in 0..reader.class_defs().len() {
            let class_def = &reader.class_defs()[class_idx];
            ...
            // Store result
            let class_name = reader.type_name(class_def.class_idx);
