...
public final class Constants {
    ...
    public static final class MessagePayloadKeys {
        ...
        public static a<String, String> extractDeveloperDefinedPayload(Bundle bundle) {
            ...
            while (iterator.hasNext()) {
                ...
                if (obj instanceof String != null && !next.startsWith("google.") && !next.startsWith("gcm.") && !next.equals("from") && !next.equals("message_type") && !next.equals("collapse_key")) {
                }
                if (!next.startsWith("google.")) {
                }
                if (!next.startsWith("gcm.")) {
                }
                if (!next.equals("from")) {
                }
                if (!next.equals("message_type")) {
                }
                if (!next.equals("collapse_key")) {
                }
                aVar.put(next, (String)(String)obj);
            }
            return aVar;
        }
    }
    ...
    static {
        Constants.WAKE_LOCK_ACQUIRE_TIMEOUT_MILLIS = TimeUnit.MINUTES.toMillis(3);
    }
}
