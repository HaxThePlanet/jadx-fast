...
public abstract class Random {
    ...
    public byte[] nextBytes(byte[] array, int fromIndex, int toIndex) {
        ...
        if (fromIndex >= 0 && fromIndex <= array.length) {
            length2 = fromIndex <= array.length ? i : i6;
        } else {
        }
        if (length2 != 0) {
            if (toIndex >= 0 && toIndex <= array.length) {
                length2 = toIndex <= array.length ? i : i6;
            } else {
            }
            i2 = length2 != 0 ? i : i6;
        } else {
    ...
    public double nextDouble(double from, double until) {
        ...
        if (Double.isInfinite(i6)) {
            ...
            if (!Double.isInfinite(from) && !Double.isNaN(from)) {
                infinite = !Double.isNaN(from) ? i2 : i4;
            } else {
            }
            if (infinite != 0) {
