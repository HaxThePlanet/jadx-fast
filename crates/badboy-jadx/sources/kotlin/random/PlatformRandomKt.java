package kotlin.random;

import java.util.Random;
import kotlin.Metadata;
import kotlin.internal.PlatformImplementations;
import kotlin.internal.PlatformImplementationsKt;
import kotlin.jvm.internal.Intrinsics;

/* loaded from: classes5.dex */
@Metadata(d1 = "\u0000\u001e\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010\u0006\n\u0000\n\u0002\u0010\u0008\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0002\u001a\t\u0010\u0000\u001a\u00020\u0001H\u0081\u0008\u001a\u0018\u0010\u0002\u001a\u00020\u00032\u0006\u0010\u0004\u001a\u00020\u00052\u0006\u0010\u0006\u001a\u00020\u0005H\u0000\u001a\u000c\u0010\u0007\u001a\u00020\u0008*\u00020\u0001H\u0007\u001a\u000c\u0010\t\u001a\u00020\u0001*\u00020\u0008H\u0007¨\u0006\n", d2 = {"defaultPlatformRandom", "Lkotlin/random/Random;", "doubleFromParts", "", "hi26", "", "low27", "asJavaRandom", "Ljava/util/Random;", "asKotlinRandom", "kotlin-stdlib"}, k = 2, mv = {1, 9, 0}, xi = 48)
public final class PlatformRandomKt {
    public static final Random asJavaRandom(kotlin.random.Random $this$asJavaRandom) {
        Object kotlinRandom;
        Intrinsics.checkNotNullParameter($this$asJavaRandom, "<this>");
        if ($this$asJavaRandom instanceof AbstractPlatformRandom) {
            kotlinRandom = $this$asJavaRandom;
        } else {
            kotlinRandom = 0;
        }
        if (kotlinRandom != null) {
            if (kotlinRandom.getImpl() == null) {
                kotlinRandom = new KotlinRandom($this$asJavaRandom);
            }
        } else {
        }
        return kotlinRandom;
    }

    public static final kotlin.random.Random asKotlinRandom(Random $this$asKotlinRandom) {
        Object platformRandom;
        Intrinsics.checkNotNullParameter($this$asKotlinRandom, "<this>");
        if ($this$asKotlinRandom instanceof KotlinRandom) {
            platformRandom = $this$asKotlinRandom;
        } else {
            platformRandom = 0;
        }
        if (platformRandom != null) {
            if (platformRandom.getImpl() == null) {
                platformRandom = new PlatformRandom($this$asKotlinRandom);
            }
        } else {
        }
        return platformRandom;
    }

    private static final kotlin.random.Random defaultPlatformRandom() {
        return PlatformImplementationsKt.IMPLEMENTATIONS.defaultPlatformRandom();
    }

    public static final double doubleFromParts(int hi26, int low27) {
        return d /= l3;
    }
}
