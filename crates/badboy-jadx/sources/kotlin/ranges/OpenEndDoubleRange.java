...
final class OpenEndDoubleRange implements kotlin.ranges.OpenEndRange<Double> {
    ...
    public boolean contains(double value) {
        int cmp;
        int i;
        if (Double.compare(value, _start) >= 0 && Double.compare(value, _endExclusive) < 0) {
            i = Double.compare(value, _endExclusive) < 0 ? 1 : 0;
        } else {
        }
        return i;
    }
    ...
    public boolean equals(Object other) {
        ...
        int i;
        int i2;
        double _endExclusive;
        double _endExclusive2;
        i = 0;
        if (other instanceof OpenEndDoubleRange) {
            i2 = 1;
            if (isEmpty()) {
                if (!(OpenEndDoubleRange)other.isEmpty()) {
                    empty = Double.compare(_endExclusive, _endExclusive2) == 0 ? i2 : i;
                    if (empty != 0) {
                        empty = Double.compare(_endExclusive, _endExclusive2) == 0 ? i2 : i;
                        if (empty != 0) {
                            i = i2;
    ...
    public boolean isEmpty() {
        int i;
        i = Double.compare(_start, _endExclusive) >= 0 ? 1 : 0;
        return i;
    }
