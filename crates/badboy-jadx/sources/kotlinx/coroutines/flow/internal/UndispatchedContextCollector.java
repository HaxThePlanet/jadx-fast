...
final class UndispatchedContextCollector<T>  implements FlowCollector<T> {
    ...
    public UndispatchedContextCollector(FlowCollector<? super T> downstream, CoroutineContext emitContext) {
        ...
        UndispatchedContextCollector.emitRef.1 anon = new UndispatchedContextCollector.emitRef.1(downstream, 0);
        this.emitRef = (Function2)anon;
    }

    public Object emit(T value, Continuation<? super Unit> $completion) {
        Object contextUndispatched = ChannelFlowKt.withContextUndispatched(this.emitContext, value, this.countOrElement, this.emitRef, $completion);
        if (contextUndispatched == IntrinsicsKt.getCOROUTINE_SUSPENDED()) {
            return contextUndispatched;
        }
