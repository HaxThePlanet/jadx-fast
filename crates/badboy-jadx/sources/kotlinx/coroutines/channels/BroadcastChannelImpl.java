...
public final class BroadcastChannelImpl<E>  extends kotlinx.coroutines.channels.BufferedChannel<E> implements kotlinx.coroutines.channels.BroadcastChannel<E> {
    ...
    public Object send(E e, Continuation<? super Unit> continuation2) {
        ...
        if (continuation2 instanceof BroadcastChannelImpl.send.1 && label &= i2 != 0) {
            $result = continuation2;
            i2 = Integer.MIN_VALUE;
            if (label &= i2 != 0) {
                $result.label = obj11 -= i2;
            } else {
        ...
        switch (label2) {
            ...
            case 1:
                ...
                ResultKt.throwOnFailure(obj11);
                subscribers = i3;
                $i$a$ForEachBroadcastChannelImpl$send$2 = $continuation;
                anon2 = $result;
                $continuation2 = obj11;
                throw l$0.getSendException();
                obj11 = $continuation2;
                $result = anon2;
                $continuation = $i$a$ForEachBroadcastChannelImpl$send$2;
                break;
            default:
