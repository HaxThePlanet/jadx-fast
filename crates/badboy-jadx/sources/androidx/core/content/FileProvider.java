...
public class FileProvider extends ContentProvider {
    ...
    static class SimplePathStrategy implements androidx.core.content.FileProvider.PathStrategy {
        ...
        public Uri getUriForFile(File file) {
            ...
            try {
                ...
                next = iterator.next();
                path2 = (File)(Map.Entry)next.getValue().getPath();
            }
            if (mostSpecific == 0) {
            } else {
                ...
                if (path5.endsWith(str)) {
                    path = canonicalPath.substring(path5.length());
                } else {
                    path = canonicalPath.substring(length3++);
                }
                StringBuilder stringBuilder2 = new StringBuilder();
                Uri.Builder builder = new Uri.Builder();
                return builder.scheme("content").authority(this.mAuthority).encodedPath(stringBuilder2.append(Uri.encode((String)mostSpecific.getKey())).append('/').append(Uri.encode(path, str)).toString()).build();
            }
            StringBuilder stringBuilder = new StringBuilder();
    ...
    private static androidx.core.content.FileProvider.PathStrategy parsePathStrategy(Context context, String authority, int resourceId) throws IOException, XmlPullParserException {
        ...
        while (next != path) {
            ...
            strArr[i2] = fileProviderPathsMetaData.getAttributeValue(externalFilesDirs, "path");
