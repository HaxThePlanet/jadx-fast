        ...
        public static androidx.core.view.DisplayCompat.ModeCompat[] getSupportedModes(Context context, Display display) {
            ...
            if (currentDisplaySizeFromWorkarounds != null) {
                if (DisplayCompat.Api23Impl.physicalSizeEquals(mode, currentDisplaySizeFromWorkarounds)) {
                    i = 0;
                    for (Object i2 : obj0) {
                        modeCompat = new DisplayCompat.ModeCompat(i2, DisplayCompat.Api23Impl.physicalSizeEquals(supportedModes[i], mode));
                        arr[i] = modeCompat;
                    }
                } else {
                    i = 0;
                    for (Object modeCompat : obj0) {
                        if (DisplayCompat.Api23Impl.physicalSizeEquals(supportedModes[i], mode)) {
                        } else {
                        }
                        physicalSizeEquals = new DisplayCompat.ModeCompat(supportedModes[i], 0);
                        arr[i] = physicalSizeEquals;
