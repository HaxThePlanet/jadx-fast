...
public final class OperationKt {
    ...
    private static final int currentNodeIndex(SlotWriter slots) {
        ...
        final int i = 1;
        current += i;
        while (current2 < currentGroup) {
            if (slots.indexInGroup(currentGroup, current2)) {
            } else {
            }
            if (slots.isNode(current2)) {
            } else {
            }
            nodeCount = slots.nodeCount(current2);
            index += nodeCount;
            current2 += groupSize;
            nodeCount = i;
            if (slots.isNode(current2)) {
    ...
    private static final void releaseMovableGroupAtCurrent(ControlledComposition composition, CompositionContext parentContext, MovableContentStateReference reference, SlotWriter slots) {
        ...
        i5 = 1;
        SlotWriter.markGroup$default(writer3, 0, i5, 0);
        writer3.update(movableContentStateReference.getParameter$runtime_release());
        java.util.List moveTo = slots.moveTo(movableContentStateReference.getAnchor$runtime_release(), i5, writer3);
        writer3.skipGroup();
        writer3.endGroup();
        writer3.endInsert();
        ...
        if (RecomposeScopeImpl.Companion.hasAnchoredRecomposeScopes$runtime_release(slotTable2, moveTo)) {
            ...
            i6 = 0;
            int i12 = 0;
            RecomposeScopeImpl.Companion.adoptAnchoredScopes$runtime_release(i2, moveTo, (RecomposeScopeOwner)anon2);
