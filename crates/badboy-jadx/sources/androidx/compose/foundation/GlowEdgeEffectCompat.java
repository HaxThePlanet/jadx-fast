...
final class GlowEdgeEffectCompat extends EdgeEffect {
    ...
    public GlowEdgeEffectCompat(Context context) {
        ...
        final int i = 0;
        final int i3 = 0;
        this.oppositeReleaseDeltaThreshold = AndroidDensity_androidKt.Density(context).toPx-0680j_4(Dp.constructor-impl((float)i2));
    }

    @Override // android.widget.EdgeEffect
    public void onAbsorb(int velocity) {
        this.oppositeReleaseDelta = 0;
    ...
    public final void releaseWithOppositeDelta(float delta) {
        this.oppositeReleaseDelta = oppositeReleaseDelta += delta;
        if (Float.compare(f, oppositeReleaseDeltaThreshold) > 0) {
            onRelease();
        }
    }
}
