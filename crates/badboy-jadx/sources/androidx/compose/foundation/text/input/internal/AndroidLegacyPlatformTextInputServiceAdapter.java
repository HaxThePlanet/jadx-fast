package androidx.compose.foundation.text.input.internal;

import androidx.compose.foundation.text.handwriting.StylusHandwriting_androidKt;
import androidx.compose.ui.geometry.Rect;
import androidx.compose.ui.graphics.Matrix;
import androidx.compose.ui.layout.LayoutCoordinates;
import androidx.compose.ui.text.TextLayoutResult;
import androidx.compose.ui.text.input.EditCommand;
import androidx.compose.ui.text.input.ImeAction;
import androidx.compose.ui.text.input.ImeOptions;
import androidx.compose.ui.text.input.OffsetMapping;
import androidx.compose.ui.text.input.TextFieldValue;
import java.util.List;
import kotlin.Metadata;
import kotlin.Unit;
import kotlin.jvm.functions.Function1;
import kotlinx.coroutines.Job;
import kotlinx.coroutines.Job.DefaultImpls;
import kotlinx.coroutines.channels.BufferOverflow;
import kotlinx.coroutines.flow.MutableSharedFlow;
import kotlinx.coroutines.flow.SharedFlowKt;

/* loaded from: classes.dex */
@Metadata(d1 = "\u0000d\n\u0002\u0018\u0002\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0002\u0010\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0005\n\u0002\u0018\u0002\n\u0002\u0008\u0002\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0010 \n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0008\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0000\n\u0002\u0018\u0002\n\u0002\u0008\u0003\u0008\u0000\u0018\u00002\u00020\u0001B\u0005¢\u0006\u0002\u0010\u0002J\u0010\u0010\r\u001a\u00020\u00052\u0006\u0010\u000e\u001a\u00020\u000fH\u0016J\u0008\u0010\u0010\u001a\u00020\u0005H\u0016J\u001e\u0010\u0010\u001a\u00020\u00052\u0014\u0010\u0011\u001a\u0010\u0012\u0004\u0012\u00020\u0007\u0012\u0004\u0012\u00020\u0005\u0018\u00010\u0012H\u0002JF\u0010\u0010\u001a\u00020\u00052\u0006\u0010\u0013\u001a\u00020\u00142\u0006\u0010\u0015\u001a\u00020\u00162\u0018\u0010\u0017\u001a\u0014\u0012\n\u0012\u0008\u0012\u0004\u0012\u00020\u00190\u0018\u0012\u0004\u0012\u00020\u00050\u00122\u0012\u0010\u001a\u001a\u000e\u0012\u0004\u0012\u00020\u001b\u0012\u0004\u0012\u00020\u00050\u0012H\u0016J\u0008\u0010\u001c\u001a\u00020\u0005H\u0016J\u0008\u0010\u001d\u001a\u00020\u0005H\u0016J\u001a\u0010\u001e\u001a\u00020\u00052\u0008\u0010\u001f\u001a\u0004\u0018\u00010\u00142\u0006\u0010 \u001a\u00020\u0014H\u0016JD\u0010!\u001a\u00020\u00052\u0006\u0010\"\u001a\u00020\u00142\u0006\u0010#\u001a\u00020$2\u0006\u0010%\u001a\u00020&2\u0012\u0010'\u001a\u000e\u0012\u0004\u0012\u00020(\u0012\u0004\u0012\u00020\u00050\u00122\u0006\u0010)\u001a\u00020\u000f2\u0006\u0010*\u001a\u00020\u000fH\u0016R\u0016\u0010\u0003\u001a\n\u0012\u0004\u0012\u00020\u0005\u0018\u00010\u0004X\u0082\u000e¢\u0006\u0002\n\u0000R\u0010\u0010\u0006\u001a\u0004\u0018\u00010\u0007X\u0082\u000e¢\u0006\u0002\n\u0000R\u0010\u0010\u0008\u001a\u0004\u0018\u00010\tX\u0082\u000e¢\u0006\u0002\n\u0000R\u001c\u0010\n\u001a\n\u0012\u0004\u0012\u00020\u0005\u0018\u00010\u00048BX\u0082\u0004¢\u0006\u0006\u001a\u0004\u0008\u000b\u0010\u000c¨\u0006+", d2 = {"Landroidx/compose/foundation/text/input/internal/AndroidLegacyPlatformTextInputServiceAdapter;", "Landroidx/compose/foundation/text/input/internal/LegacyPlatformTextInputServiceAdapter;", "()V", "backingStylusHandwritingTrigger", "Lkotlinx/coroutines/flow/MutableSharedFlow;", "", "currentRequest", "Landroidx/compose/foundation/text/input/internal/LegacyTextInputMethodRequest;", "job", "Lkotlinx/coroutines/Job;", "stylusHandwritingTrigger", "getStylusHandwritingTrigger", "()Lkotlinx/coroutines/flow/MutableSharedFlow;", "notifyFocusedRect", "rect", "Landroidx/compose/ui/geometry/Rect;", "startInput", "initializeRequest", "Lkotlin/Function1;", "value", "Landroidx/compose/ui/text/input/TextFieldValue;", "imeOptions", "Landroidx/compose/ui/text/input/ImeOptions;", "onEditCommand", "", "Landroidx/compose/ui/text/input/EditCommand;", "onImeActionPerformed", "Landroidx/compose/ui/text/input/ImeAction;", "startStylusHandwriting", "stopInput", "updateState", "oldValue", "newValue", "updateTextLayoutResult", "textFieldValue", "offsetMapping", "Landroidx/compose/ui/text/input/OffsetMapping;", "textLayoutResult", "Landroidx/compose/ui/text/TextLayoutResult;", "textFieldToRootTransform", "Landroidx/compose/ui/graphics/Matrix;", "innerTextFieldBounds", "decorationBoxBounds", "foundation_release"}, k = 1, mv = {1, 8, 0}, xi = 48)
public final class AndroidLegacyPlatformTextInputServiceAdapter extends androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter {

    public static final int $stable = 8;
    private MutableSharedFlow<Unit> backingStylusHandwritingTrigger;
    private androidx.compose.foundation.text.input.internal.LegacyTextInputMethodRequest currentRequest;
    private Job job;
    static {
        final int i = 8;
    }

    public static final MutableSharedFlow access$getStylusHandwritingTrigger(androidx.compose.foundation.text.input.internal.AndroidLegacyPlatformTextInputServiceAdapter $this) {
        return $this.getStylusHandwritingTrigger();
    }

    public static final void access$setCurrentRequest$p(androidx.compose.foundation.text.input.internal.AndroidLegacyPlatformTextInputServiceAdapter $this, androidx.compose.foundation.text.input.internal.LegacyTextInputMethodRequest <set-?>) {
        $this.currentRequest = <set-?>;
    }

    public static final void access$startInput$localToScreen(androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter.LegacyPlatformTextInputNode node, float[] matrix) {
        AndroidLegacyPlatformTextInputServiceAdapter.startInput$localToScreen(node, matrix);
    }

    private final MutableSharedFlow<Unit> getStylusHandwritingTrigger() {
        final MutableSharedFlow backingStylusHandwritingTrigger = this.backingStylusHandwritingTrigger;
        if (backingStylusHandwritingTrigger != null) {
            return backingStylusHandwritingTrigger;
        }
        int i = 0;
        if (!StylusHandwriting_androidKt.isStylusHandwritingSupported()) {
            return i;
        }
        MutableSharedFlow mutableSharedFlow = SharedFlowKt.MutableSharedFlow$default(1, 0, BufferOverflow.DROP_LATEST, 2, i);
        int i3 = 0;
        this.backingStylusHandwritingTrigger = mutableSharedFlow;
        return mutableSharedFlow;
    }

    private final void startInput(Function1<? super androidx.compose.foundation.text.input.internal.LegacyTextInputMethodRequest, Unit> initializeRequest) {
        final androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter.LegacyPlatformTextInputNode textInputModifierNode = getTextInputModifierNode();
        if (textInputModifierNode == null) {
        }
        AndroidLegacyPlatformTextInputServiceAdapter.startInput.2 anon = new AndroidLegacyPlatformTextInputServiceAdapter.startInput.2(initializeRequest, this, textInputModifierNode, 0);
        this.job = textInputModifierNode.launchTextInputSession((Function2)anon);
    }

    private static final void startInput$localToScreen(androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter.LegacyPlatformTextInputNode node, float[] matrix) {
        LayoutCoordinates layoutCoordinates;
        boolean it;
        int i;
        layoutCoordinates = node.getLayoutCoordinates();
        if (layoutCoordinates != null) {
            i = 0;
            if (layoutCoordinates.isAttached()) {
            } else {
                layoutCoordinates = 0;
            }
            if (layoutCoordinates == 0) {
            }
            layoutCoordinates.transformToScreen-58bKbWc(matrix);
        }
    }

    @Override // androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter
    public void notifyFocusedRect(Rect rect) {
        final androidx.compose.foundation.text.input.internal.LegacyTextInputMethodRequest currentRequest = this.currentRequest;
        if (currentRequest != null) {
            currentRequest.notifyFocusedRect(rect);
        }
    }

    @Override // androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter
    public void startInput() {
        startInput(0);
    }

    public void startInput(TextFieldValue value, ImeOptions imeOptions, Function1<? super List<? extends EditCommand>, Unit> onEditCommand, Function1<? super ImeAction, Unit> onImeActionPerformed) {
        AndroidLegacyPlatformTextInputServiceAdapter.startInput.1 anon = new AndroidLegacyPlatformTextInputServiceAdapter.startInput.1(value, this, imeOptions, onEditCommand, onImeActionPerformed);
        startInput((Function1)anon);
    }

    @Override // androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter
    public void startStylusHandwriting() {
        Unit iNSTANCE;
        final MutableSharedFlow stylusHandwritingTrigger = getStylusHandwritingTrigger();
        if (stylusHandwritingTrigger != null) {
            stylusHandwritingTrigger.tryEmit(Unit.INSTANCE);
        }
    }

    @Override // androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter
    public void stopInput() {
        int i;
        Job job = this.job;
        final int i2 = 0;
        if (job != null) {
            Job.DefaultImpls.cancel$default(job, i2, 1, i2);
        }
        this.job = i2;
        MutableSharedFlow stylusHandwritingTrigger = getStylusHandwritingTrigger();
        if (stylusHandwritingTrigger != null) {
            stylusHandwritingTrigger.resetReplayCache();
        }
    }

    @Override // androidx.compose.foundation.text.input.internal.LegacyPlatformTextInputServiceAdapter
    public void updateState(TextFieldValue oldValue, TextFieldValue newValue) {
        final androidx.compose.foundation.text.input.internal.LegacyTextInputMethodRequest currentRequest = this.currentRequest;
        if (currentRequest != null) {
            currentRequest.updateState(oldValue, newValue);
        }
    }

    public void updateTextLayoutResult(TextFieldValue textFieldValue, OffsetMapping offsetMapping, TextLayoutResult textLayoutResult, Function1<? super Matrix, Unit> textFieldToRootTransform, Rect innerTextFieldBounds, Rect decorationBoxBounds) {
        TextFieldValue textFieldValue2;
        OffsetMapping map;
        TextLayoutResult textLayoutResult2;
        Rect rect;
        Rect rect2;
        final androidx.compose.foundation.text.input.internal.LegacyTextInputMethodRequest currentRequest = this.currentRequest;
        if (currentRequest != null) {
            currentRequest.updateTextLayoutResult(textFieldValue, offsetMapping, textLayoutResult, innerTextFieldBounds, decorationBoxBounds);
        } else {
            textFieldValue2 = textFieldValue;
            map = offsetMapping;
            textLayoutResult2 = textLayoutResult;
            rect = innerTextFieldBounds;
            rect2 = decorationBoxBounds;
        }
    }
}
