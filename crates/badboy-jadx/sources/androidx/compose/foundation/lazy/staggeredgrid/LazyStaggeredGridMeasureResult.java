...
public final class LazyStaggeredGridMeasureResult implements androidx.compose.foundation.lazy.staggeredgrid.LazyStaggeredGridLayoutInfo, MeasureResult {
    ...
    public final boolean getCanScrollBackward() {
        ...
        int i2;
        i2 = 0;
        if (this.firstVisibleItemIndices[i2] == 0) {
            if (this.firstVisibleItemScrollOffsets[i2] > 0) {
                i2 = 1;
            }
        } else {
        }
    ...
    public final boolean tryToApplyScrollWithoutRemeasure(int delta) {
        ...
        if (!this.remeasureNeeded && !getVisibleItemsInfo().isEmpty()) {
            if (!getVisibleItemsInfo().isEmpty()) {
                i2 = 1;
                remeasureNeeded = firstVisibleItemIndices.length == 0 ? i2 : i;
                if (remeasureNeeded == 0) {
                    remeasureNeeded = firstVisibleItemScrollOffsets.length == 0 ? i2 : i;
                    if (remeasureNeeded != 0) {
                    } else {
                        List visibleItemsInfo2 = getVisibleItemsInfo();
                        int i6 = 0;
                        index$iv = 0;
                        while (index$iv < visibleItemsInfo2.size()) {
                            obj = obj2;
                            i3 = 0;
                        ...
                        int i5 = 0;
                        index$iv2 = 0;
                        while (index$iv2 < visibleItemsInfo3.size()) {
                            obj = 0;
                            (LazyStaggeredGridMeasuredItem)visibleItemsInfo3.get(index$iv2).applyScrollDelta(delta);
                            index$iv2++;
                        }
