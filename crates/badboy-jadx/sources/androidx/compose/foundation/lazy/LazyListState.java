...
public final class LazyListState implements ScrollableState {
    ...
    public final float onScroll$foundation_release(float distance) {
        ...
        if (i2 == 0) {
            ...
            if (Float.compare(f2, i7) > 0 && (LazyListMeasureResult)value.tryToApplyScrollWithoutRemeasure($this$fastRoundToInt$iv, hasLookaheadPassOccurred ^= i) && postLookaheadLayoutInfo != null) {
                value = this.layoutInfoState.getValue();
                scrollToBeConsumed = this.scrollToBeConsumed;
                i8 = 0;
                $this$fastRoundToInt$iv = Math.round(this.scrollToBeConsumed);
                postLookaheadLayoutInfo = this.postLookaheadLayoutInfo;
                if ((LazyListMeasureResult)value.tryToApplyScrollWithoutRemeasure($this$fastRoundToInt$iv, hasLookaheadPassOccurred ^= i)) {
                    if (postLookaheadLayoutInfo != null) {
                        scrolledWithoutRemeasure = postLookaheadLayoutInfo.tryToApplyScrollWithoutRemeasure($this$fastRoundToInt$iv, i);
                    }
                }
                if (scrolledWithoutRemeasure) {
                    applyMeasureResult$foundation_release(value, this.hasLookaheadPassOccurred, i);
    ...
    public Object scroll(MutatePriority mutatePriority, Function2<? super ScrollScope, ? super Continuation<? super Unit>, ? extends Object> function22, Continuation<? super Unit> continuation3) {
        ...
        if (continuation3 instanceof LazyListState.scroll.1 && label2 &= i3 != 0) {
            anon = continuation3;
            i3 = Integer.MIN_VALUE;
            if (label2 &= i3 != 0) {
                anon.label = obj9 -= i3;
