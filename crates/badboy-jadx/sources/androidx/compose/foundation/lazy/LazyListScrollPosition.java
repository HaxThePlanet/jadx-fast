...
public final class LazyListScrollPosition {
    ...
    public LazyListScrollPosition(int initialIndex, int initialScrollOffset) {
        ...
        LazyLayoutNearestRangeState lazyLayoutNearestRangeState = new LazyLayoutNearestRangeState(initialIndex, 30, 100);
        this.nearestRangeState = lazyLayoutNearestRangeState;
    }

    public LazyListScrollPosition(int i, int i2, int i3, DefaultConstructorMarker defaultConstructorMarker4) {
        int obj2;
        int obj3;
        final int i4 = 0;
        obj2 = i3 & 1 != 0 ? i4 : obj2;
    ...
    public final void updateFromMeasureResult(androidx.compose.foundation.lazy.LazyListMeasureResult measureResult) {
        ...
        if (!this.hadFirstNotEmptyLayout) {
            if (measureResult.getTotalItemsCount() > 0) {
                ...
                firstVisibleItemScrollOffset = measureResult.getFirstVisibleItemScrollOffset();
                ...
                if (i == 0) {
                    ...
                    hadFirstNotEmptyLayout = measureResult.getFirstVisibleItem();
                    if (hadFirstNotEmptyLayout != null) {
                        index = hadFirstNotEmptyLayout.getIndex();
                    }
                    update(index, firstVisibleItemScrollOffset);
                }
            }
        } else {
        }
        int i2 = 0;
        StringBuilder stringBuilder = new StringBuilder();
        IllegalStateException illegalStateException = new IllegalStateException(stringBuilder.append("scrollOffset should be non-negative (").append(firstVisibleItemScrollOffset).append(')').toString().toString());
