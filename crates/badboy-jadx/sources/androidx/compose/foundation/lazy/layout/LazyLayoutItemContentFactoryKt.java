...
public final class LazyLayoutItemContentFactoryKt {
    private static final void SkippableItem-JVlU9Rs(androidx.compose.foundation.lazy.layout.LazyLayoutItemProvider itemProvider, Object saveableStateHolder, int index, Object key, Composer $composer, int $changed) {
        ...
        if ($changed & 6 == 0) {
            i4 = obj12.changed(itemProvider) ? 4 : 2;
            $dirty |= i4;
        }
        if ($changed & 48 == 0) {
            i7 = obj12.changed(saveableStateHolder) ? 32 : 16;
            $dirty |= i7;
        }
        if ($changed & 384 == 0) {
            i8 = obj12.changed(index) ? 256 : 128;
            $dirty |= i8;
        }
        if ($changed & 3072 == 0) {
            i5 = obj12.changed(key) ? 2048 : 1024;
            $dirty |= i5;
        }
        if ($dirty & 1171 == 1170) {
            if (!obj12.getSkipping()) {
                if (ComposerKt.isTraceInProgress()) {
                    ComposerKt.traceEventStart(traceInProgress2, $dirty, -1, "androidx.compose.foundation.lazy.layout.SkippableItem (LazyLayoutItemContentFactory.kt:132)");
                }
