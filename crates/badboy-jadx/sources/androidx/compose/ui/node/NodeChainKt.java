...
public final class NodeChainKt {
    ...
    private static final MutableVector<Modifier.Element> fillVector(Modifier $this$fillVector, MutableVector<Modifier.Element> result) {
        ...
        while (mutableVector.isNotEmpty()) {
            i = mutableVector.removeAt(size2--);
            if (i instanceof CombinedModifier) {
            } else {
            }
            if (i instanceof Modifier.Element != 0) {
            } else {
            }
            if (it == 0) {
            } else {
            ...
            it = outer$ui_release;
            result.add(i);
            mutableVector.add((CombinedModifier)i.getInner$ui_release());
            mutableVector.add((CombinedModifier)i.getOuter$ui_release());
        }
        return result;
    }
