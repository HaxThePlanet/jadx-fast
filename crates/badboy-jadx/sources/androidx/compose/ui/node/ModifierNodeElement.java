...
public abstract class ModifierNodeElement<N extends androidx.compose.ui.Modifier.Node>  implements Modifier.Element, InspectableValue {
    ...
    private final InspectorInfo getInspectorValues() {
        InspectorInfo _inspectorValues;
        InspectorInfo inspectorInfo;
        int i;
        String simpleName;
        if (this._inspectorValues == null) {
            _inspectorValues = new InspectorInfo();
            inspectorInfo = _inspectorValues;
            int i2 = 0;
            inspectorInfo.setName(Reflection.getOrCreateKotlinClass(getClass()).getSimpleName());
            inspectableProperties(inspectorInfo);
            i = 0;
            this._inspectorValues = inspectorInfo;
        }
        return _inspectorValues;
    }
    ...
    public final Sequence<ValueElement> getInspectableElements() {
        return (Sequence)getInspectorValues().getProperties();
    }
