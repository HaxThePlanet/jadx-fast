...
public final class TextLayout_androidKt {
    ...
    private static final long getLineHeightPaddings(LineHeightStyleSpan[] $this$getLineHeightPaddings) {
        ...
        while (i < $this$getLineHeightPaddings.length) {
            ...
            i++;
            lastDescentDiff = Math.max(firstAscentDiff, Math.abs(obj.getLastDescentDiff()));
            firstAscentDiff = Math.max(firstAscentDiff, Math.abs(obj.getFirstAscentDiff()));
        }
        if (firstAscentDiff == 0 && lastDescentDiff == 0) {
            if (lastDescentDiff == 0) {
                zeroVerticalPadding = TextLayout_androidKt.ZeroVerticalPadding;
            } else {
                zeroVerticalPadding = TextLayout_androidKt.VerticalPaddings(firstAscentDiff, lastDescentDiff);
            }
        } else {
        }
