...
public final class ScrollCapture_androidKt {
    ...
    private static final void visitScrollCaptureCandidates(SemanticsNode fromNode, int depth, Function1<? super androidx.compose.ui.scrollcapture.ScrollCaptureCandidate, Unit> onCandidate) {
        List childrenForSearch;
        MutableVector childrenForSearch2;
        int i;
        int candidateDepth;
        ...
        childrenForSearch2 = mutableVector2;
        i = 0;
        childrenForSearch2.addAll(childrenForSearch2.getSize(), ScrollCapture_androidKt.getChildrenForSearch(fromNode));
        while (mutableVector2.isNotEmpty()) {
            int i5 = 0;
            childrenForSearch = mutableVector2.removeAt(size -= candidateDepth);
            childrenForSearch2 = childrenForSearch;
            i = 0;
            candidateDepth = i9;
            if (candidateDepth != 0) {
            }
            i = mutableVector2;
            coordinates = 0;
            i.addAll(i.getSize(), ScrollCapture_androidKt.getChildrenForSearch((SemanticsNode)childrenForSearch));
            NodeCoordinator coordinatorToGetBounds$ui_release = childrenForSearch2.findCoordinatorToGetBounds$ui_release();
            int i10 = 0;
            ...
            if (!ScrollCapture_androidKt.getCanScrollVertically(childrenForSearch2)) {
            } else {
            }
            int i8 = depth + 1;
            scrollCaptureCandidate = new ScrollCaptureCandidate(childrenForSearch2, i8, roundToIntRect, coordinates);
            onCandidate.invoke(scrollCaptureCandidate);
            ScrollCapture_androidKt.visitScrollCaptureCandidates(childrenForSearch2, i8, onCandidate);
            candidateDepth = i9;
            candidateDepth = i9;
        }
    }
