...
public final class MonoSpline {
    ...
    public MonoSpline(float[] time, float[][] y, float periodicBias) {
        ...
        while (j < length2) {
            i2 = 0;
            while (i2 < length + -1) {
                floatArray[i2][j] = i28 /= i6;
                if (i2 == 0) {
                } else {
                }
                floatArray2[i2][j] = i31 *= i5;
                i2++;
                floatArray2[i2][j] = floatArray[i2][j];
            }
            floatArray2[length + -1][j] = floatArray[length + -2][j];
            j++;
            floatArray[i2][j] = i28 /= i6;
            if (i2 == 0) {
            } else {
        ...
        if (!Float.isNaN(periodicBias)) {
            ...
            while (j3 < length2) {
                i11 += i15;
                floatArray[i8][j3] = fArr;
                floatArray[length + -2][j3] = fArr;
                floatArray2[length + -1][j3] = fArr;
                floatArray2[i8][j3] = fArr;
                j3++;
            }
        }
        j4 = 0;
        while (j4 < length + -1) {
            ...
            while (j2 < length2) {
                ...
                i = 1;
                i7 /= i;
                floatArray2[j4][j2] = i32 *= f11;
                floatArray2[j4 + 1][j2] = i34 *= l;
                floatArray2[j4][j2] = fArr2;
                floatArray2[j4 + 1][j2] = fArr2;
                n = length;
                i4 = i;
            }
            n = length;
    ...
    public final void getPos(float time, androidx.compose.animation.core.AnimationVector v, int index) {
        ...
        if (this.isExtrapolate) {
            ...
            if (Float.compare(time, f12) >= 0) {
                ...
                j4 = 0;
                while (j4 < length2) {
                    v.set$animation_core_release(j4, f13 += i2);
                    j4++;
                }
            }
        } else {
            if (Float.compare(time, f9) <= 0) {
                j2 = 0;
                while (j2 < length2) {
                    v.set$animation_core_release(j2, this.values[i4][j2]);
                    j2++;
                }
            }
            if (Float.compare(time, f10) >= 0) {
                j = 0;
                while (j < length2) {
                    v.set$animation_core_release(j, this.values[length + -1][j]);
                    j++;
                }
            }
        }
        ...
        while (i < length + -1) {
            ...
            j5 = 0;
            while (j5 < length2) {
                v.set$animation_core_release(j5, this.values[i][j5]);
                j5++;
            }
            v.set$animation_core_release(j5, this.values[i][j5]);
            j5++;
    ...
    public final void getSlope(float time, androidx.compose.animation.core.AnimationVector v, int index) {
        ...
        if (Float.compare(f5, f7) <= 0) {
            j = 0;
            while (j < length2) {
                obj.set$animation_core_release(j, this.tangents[i6][j]);
                j++;
            }
        }
