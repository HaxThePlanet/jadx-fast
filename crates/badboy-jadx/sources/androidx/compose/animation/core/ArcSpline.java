    ...
    public static final class Arc {
        ...
        private final void buildTable(float x1, float y1, float x2, float y2) {
            ...
            while (i4 < fArr2.length) {
                ...
                a = i3;
                i2 = i7;
                ArcSpline.Arc.Companion.access$getOurPercent(ArcSpline.Arc.Companion)[i4] = dist += f4;
            }
            int a3 = i3;
            int i23 = i7;
            ...
            while (i6 < fArr.length) {
                i4 = ArcSpline.Arc.Companion.access$getOurPercent(ArcSpline.Arc.Companion);
                i4[i6] = i12 /= dist;
                i6++;
            }
            i = 0;
            while (i < lut2.length) {
                f3 = f5 / f6;
                i4 = ArraysKt.binarySearch$default(ArcSpline.Arc.Companion.access$getOurPercent(ArcSpline.Arc.Companion), f3, 0, 0, 6, 0);
                if (i4 >= 0) {
                } else {
                }
                if (i4 == -1) {
    ...
    public ArcSpline(int[] arcModes, float[] timePoints, float[][] y) {
        ...
        while (i10 < i17) {
            ...
            while (i2 < i16) {
                i3 = i2 * 2;
                int i22 = i21;
                arc = new ArcSpline.Arc(i, iArr[i10], iArr[i22], y[i10][i3], y[i10][i3 + 1], y[i22][i3], y[i22][i3 + 1]);
                arr[i2] = arc;
                i2++;
            }
            arr2[i10] = arr;
            ...
            if (Float.compare(t, time12) >= 0) {
                ...
                while (i4 < arcs6.length) {
                    ...
                    while (j < v.length) {
                        ...
                        j += 2;
                        k++;
                        if (this.arcs[i4][k].isLinear()) {
                        } else {
                        }
                        this.arcs[i4][k].setPoint(t);
                        v[j] = this.arcs[i4][k].calcX();
                        v[j + 1] = this.arcs[i4][k].calcY();
                        populated = 1;
                        v[j] = this.arcs[i4][k].getLinearX(t);
                        v[j + 1] = this.arcs[i4][k].getLinearY(t);
                        populated = 1;
                    }
                    i4++;
                    if (Float.compare(t, time24) <= 0) {
