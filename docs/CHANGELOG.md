...
|----|-----|----------------|-------------|
| **BUG-001** | Undefined switch variable `i` | 6+ | Switch map synthetic classes (AnonymousClass1) not generated |
| **BUG-002** | Undefined variables `d`, `d2` | 10+ | Division expressions reference undefined variables |
...
| **BUG-010** | Static final field reassignment | Invalid Java - final fields cannot be reassigned |
| **BUG-011** | 712 missing AnonymousClass | Major functionality loss (JADX: 713, Dexterity: 1) |
| **BUG-012** | Variable type reassignment | Type safety violations |

#### Positive Findings

- **Variable naming preserves debug info** - Uses `savedInstanceState` instead of generic `bundle`
- **Long literal handling correct** - Uses `0L` vs `0`
...
**The Problem:**
Kotlin Compose UI methods with compiler-generated bytecode produced 939+ lines of unreadable garbage code instead of clean decompilation.

**Solution:**
Added `should_skip_complex_method()` function in `method_gen.rs` with three detection strategies:
