...
**Root Cause:** Region builder was including merged condition blocks in both the condition prelude AND the then-body, causing code duplication
**Example:** BadAccessibilityService.java went from 96 lines to 85 lines (-11.5%), fixing duplicate Log.d() calls

**Fix Applied:**
- `crates/dexterity-passes/src/region_builder.rs` - In `process_if()`:
  - Mark ALL merged condition blocks as processed (not just the first)
  - Filter merged blocks from `then_blocks` to avoid re-traversing
  - This ensures code from merged blocks is only generated by `emit_condition_block_prelude()` in codegen

**Validation:** All 686 integration tests pass, BadAccessibilityService.java duplicate code eliminated
